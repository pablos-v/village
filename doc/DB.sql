--
-- ============================================================================
-- ВНИМАНИЕ! ЭТО REFERENCE-ДАМП, НЕ ДЛЯ ИМПОРТА.
-- ============================================================================
--
-- Этот файл — справочник по структуре существующей БД (правленный pg_dump).
-- Не загружайте его как есть: он содержит расхождения и неполную итоговую схему.
--
-- Известные расхождения с CREATE TABLE:
--   * payment INSERT'ы используют 5 значений, в CREATE TABLE 4 колонки
--     (поле `amount` будет добавлено по плану — см. doc/ARCHITECTURE.md)
--   * FK household_inhabitant_fk ссылается на поле inh_id, в схеме master_inh_id
--     (по итоговому плану этот FK удаляется — см. п.9 doc/README.md)
--   * payment.paydate имеет тип date, но в данных целочисленные значения
--
-- Итоговая нормализованная схема описана в doc/ARCHITECTURE.md и doc/README.md.
-- Реализация — через Liquibase changelog с нуля, в схеме `village`.
--
-- Использовать этот файл только для:
--   1) понимания существующих таблиц street/bldng/address/household/inhabitant/events/payment;
--   2) копирования тестовых данных в Liquibase loadData / SQL-fixture (после правки).
-- ============================================================================
