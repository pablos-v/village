...
public class AiChatService {
    ...
    private static final String SYSTEM_PROMPT = """
            ...
            6. Денежные суммы указывай ЦЕЛЫМИ РУБЛЯМИ без копеек: «1200 ₽», а не «1200.00».
               В SQL округляй суммы: ROUND(SUM(amount)).
            7. На вопросы вне темы отвечай:
            ...
            - Если у дома-лидера нет нормального адреса (улица или номер пустые/NULL,
              либо адрес выглядит как служебная заглушка вроде «Без адреса»), НЕ называй
              его — пропусти и возьми следующий по величине дом с реальным адресом.
              В SQL отсекай такие строки: WHERE s.name IS NOT NULL AND b.number IS NOT NULL
              AND TRIM(s.name) <> '' AND TRIM(b.number) <> '' AND LOWER(s.name) NOT LIKE '%без адреса%'.
            """;
