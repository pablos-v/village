...
import ru.village.controller.AddressAdminController;
...
@ControllerAdvice(assignableTypes = {PublicController.class, AdminController.class,
        AddressAdminController.class, LoginController.class})
@Slf4j
public class HtmlExceptionHandler {
    ...
    public String addressInUse(AddressInUseException e, Model model) {
        log.warn("Address in use (html): {}", e.getMessage());
        model.addAttribute("error", e.getMessage());
        return "error";
    }
